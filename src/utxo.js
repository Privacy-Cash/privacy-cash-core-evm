...
const NATIVE_ETH_MINT = BigNumber.from(0)
...
class Utxo {
   ...
   */
  constructor({ amount = 0, keypair = new Keypair(), blinding = randomBN(), index = null, mintAddress = NATIVE_ETH_MINT } = {}) {
  ...
  getCommitment() {
    if (!this._commitment) {
      this._commitment = poseidonHash([this.amount, this.keypair.pubkey, this.blinding, this.mintAddress])
    }
